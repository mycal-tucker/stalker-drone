...
from pyparrot_test.Minidrone import Mambo
from pyparrot_test.DroneVision import DroneVision
import threading
import cv2
import time
...
if (success):
    ...
    if (success):
        ...
        if (testFlying):
            ...
            current_vel = mambo.get_velocity()
            print("first current velocity reading is ", current_vel)

            if (mambo.sensors.flying_state != "emergency"):
                print("flying state is %s" % mambo.sensors.flying_state)
                print("Flying direct: going up")
                mambo.fly_direct(roll=0, pitch=0, yaw=0, vertical_movement=20, duration=1)
                test_vel = mambo.get_velocity()
                print("New velocity reading is ", test_vel)
